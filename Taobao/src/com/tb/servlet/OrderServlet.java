...
public class OrderServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
	    ...
	    if(request.getSession().getAttribute("sUser")==null){
	    	JsUtils.jsAlertLocation(out, "请登录", "login");
	    	return;
	    }
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
	    ...
	    if(oname.isEmpty()){
	    	JsUtils.jsAlertBack(out, "名字不能为空");
	    	return;
	    }
	    if(otel.isEmpty()||otel.length()<11){
	    	JsUtils.jsAlertBack(out, "电话不能为空或电话格式不正确");
	    	return;
	    }
	    
	    if(oaddress.isEmpty()){
	    	JsUtils.jsAlertBack(out, "地址不能为空");
	    	return;
	    }
	    ...
	    if(oid>0){
	    	JsUtils.jsAlertLocation(out, "下单成功，订单号："+oid, "successorder");
	    }else{
	    	JsUtils.jsAlertBack(out, "下单失败");
	    }
